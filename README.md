...
A record may declare the `uid` it is written with, which is what makes a seeded
page tree the same tree in every installation. Because a suggested uid is a
suggestion and not a demand, an import refuses to run when one of them is
already taken and names the records in the way — a deleted record occupies its
uid as much as any other one.
...
**Feature complete, and pre-1.0.** Both commands, the definition format, record
seeding, file seeding and site configurations are implemented, covered by unit
and functional tests, and green on TYPO3 v13.4 and v14.3.
...
interface is the YAML format and the two commands.
...
- Translations are writable as ordinary fields; a first-class translation
  construct is a later feature.
...
needs no support in the extension to be seedable. The complete key reference is
in [`Documentation/Configuration/`](Documentation/Configuration) for integrators
and in
