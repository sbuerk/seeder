...
final class RecordSeederTest extends AbstractFunctionalTestCase
{
     ...
     * a standard page.
     *
     * The last assertion is what makes this more than a smoke test.
     * `CreateSiteConfiguration` runs after every new page and reads
     ...
     * versions. Proving that the automatic site configuration was written
     * proves that the hook was not only registered but walked past every one of
     * those reads, with the suite turning a warning into a failure. A core
     * version that starts reading a field `DataMapFactory` does not write shows
     * up here.
     *
     ...
     *
     * @todo The automatic site configuration is suppressed once the seeder sets
     *       `DataHandler::$isImporting`, see #10. The `isImporting` check is the
     *       *last* condition of the hook's early return, so the three reads
     *       still happen and the rest of this test stays as it is - only the
     *       assertion on the written site configuration is then inverted.
     */
    #[Test]
    public function aPageDeclaringNothingButATitleIsSeededComplete(): void
    {
        ...
        // The hook ran through all of its reads and wrote the site
        // configuration TYPO3 creates for a new root page.
        $this->assertCount(
            1,
            glob(Environment::getConfigPath() . '/sites/autogenerated-*') ?: [],
            'The site configuration hook did not run, so this test proves nothing about the fields it reads.',
        );
    }
