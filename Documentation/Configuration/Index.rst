...
tables whose disable field TYPO3 enriches default to ``0``. That is why every
example above declares :yaml:`defaultValues: {hidden: 0}`: it costs nothing on
the tables that already default to ``0``, and it is what keeps a seeded page
tree visible.
