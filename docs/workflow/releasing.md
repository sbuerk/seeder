...
This branch line is **`main`** — the branch carrying TYPO3 v13.4 + v14.3, which
releases the 2.x line. `1` carries the 1.x line, TYPO3 v12.4 + v13.4. The two
lines are released independently, and neither branch is merged into the other.

Both scripts default `--source-branch` to the branch they sit on — `main` here,
`1` on branch `1`:
...
> releasing `1` from a `main` checkout swaps `release.sh` and `setVersion.sh`
...
Build/Scripts/setVersion.sh 2.1.0 dev --source-branch=main
...
and tags. Passing `--source-branch=1` from a `main` checkout is therefore *not*
the same as working on `1` — see the note in
...
[CI matrix](../../.github/workflows/ci.yml) — 36 jobs on `main`, 35 on `1` as
...
```bash
Build/Scripts/setVersion.sh 2.1.0 dev --source-branch=main
git commit -am "[TASK] Set version 2.1.0"
```

`dev` writes `2.1.0-dev` into `VERSION`, `composer.json` and
`Build/Scripts/runTests.sh`, `2.1.0` into `ext_emconf.php`, and sets
`extra.branch-alias` to `dev-main: 2.1.x-dev`. The same command with a different
