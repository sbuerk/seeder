...
1. Create a topic branch off the branch the change belongs on — for example
   ...
   `1`**, which carries TYPO3 v12.4 + v13.4; `main` carries the 2.x line for
   TYPO3 v13.4 + v14.3. Both release scripts default `--source-branch` to the
...
5. Open the pull request against the branch the topic branch was cut from —
   `1` for a change to the TYPO3 v12 + v13 line, `main` for the v13 + v14 line
   — describing what changes and why. The
   [CI workflow](../../.github/workflows/ci.yml) runs the full matrix for TYPO3
   v12 and v13 on this branch, and comments the rendered documentation on the
   pull request —
