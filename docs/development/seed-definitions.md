...
The `pages` TCA defaults `hidden` to `1` on **both** supported core versions, so
a page written without a `hidden` value is **hidden**. Where that default is
declared differs and does not matter: 12.4 writes it into the `hidden` column of
the shipped `EXT:core/Configuration/TCA/pages.php`, 13.4 has no `hidden` column
in that file and sets
`$GLOBALS['TCA']['pages']['columns']['hidden']['config']['default'] = 1` from
`EXT:core/Configuration/TCA/Overrides/pages.php` instead. `tt_content` and the
tables whose disabled field 13.4 enriches default to `0`.
